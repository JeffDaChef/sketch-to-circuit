...
def test_rc_cutoff_is_minus_3db_and_minus_45_deg():
    ...
    assert abs(h) == pytest.approx(1 / math.sqrt(2), rel=1e-6)
...
def test_lowpass_passes_dc_blocks_high_freq():
    n, R, C = rc_lowpass()
    fc = 1.0 / (2 * math.pi * R * C)
    assert abs(solve_ac(n, fc / 100).gain("out")) > 0.99
    assert abs(solve_ac(n, fc * 100).gain("out")) < 0.02
...
def test_series_rlc_bandpass_peaks_at_resonance():
    ...
    assert at_res == pytest.approx(1.0, abs=1e-6)
...
def test_inductor_is_short_at_low_freq_open_at_high():
    ...
    assert abs(solve_ac(n, 1.0).gain("out")) < 0.05
    assert abs(solve_ac(n, 1e7).gain("out")) > 0.95
...
def test_reverse_biased_diode_is_high_resistance():
    ...
    n.add("V", "vin", 0.0, "in", "0")
    n.add("D", "D1", 0.0, "in", "mid")
    n.add("R", "Rpull", 1000.0, "mid", "0")
    n.add("I", "Irev", 1e-6, "0", "mid")
...
def test_bias_tunable_lowpass_cutoff_tracks_bias():
    ...
    for ibias in (1e-4, 1e-3):
        ...
        assert sweep["mag"][0] == pytest.approx(1 / math.sqrt(2), rel=0.02)
    assert fcs[1] / fcs[0] == pytest.approx(10.0, rel=0.05)
