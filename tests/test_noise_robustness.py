...
@pytest.fixture
def img():
    # A mid-grey RGB image is enough to test the corruption maths.
    return np.full((40, 50, 3), 128, dtype=np.uint8)


@pytest.fixture(scope="module")
def small_bases():
    # Rendering full-size synthetic images is the slow part, so render a tiny set
    # ONCE and share it across the integration tests in this file.
...
def test_corruptions_are_deterministic_per_seed(img):
    # Same seed -> identical output (the whole study's reproducibility rests on this).
...
def test_speckle_is_monotonic_superset(img):
    # A higher amount must corrupt a strict superset of the pixels a lower one did,
    # which is what guarantees the accuracy curve only ever goes down, not up.
    ...
    assert np.all(changed_high[changed_low])              # every low-changed pixel is also high-changed


def test_more_blur_changes_more(img):
    # Put an edge in the image so blur has something to smear.
    edged = img.copy()
    edged[:, :25] = 30
...
def test_study_structure_and_clean_baseline_is_perfect(small_bases):
    # Use a minimal 2-level corruption so the integration test stays fast while
    # still exercising run_study's structure. Level 0 (clean) must be 100%, matching
    # the 200/200 clean baseline.
    ...
    assert data["accuracy"][0] == pytest.approx(100.0)        # level 0 is the clean image
...
def test_score_returns_correct_total(small_bases):
    ...
    assert total == 3 and correct == 3                        # clean -> all correct
